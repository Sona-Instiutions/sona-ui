...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const bannerImageUrl = blog.bannerImage?.formats?.large?.url
  ...
  // ✅ FETCH RELATED BLOGS
  const relatedBlogs = await getRelatedBlogs(blog.categories?.[0]?.slug, blog.slug, blog.tags?.[0]?.slug);
  ...
  return (
    <div>
      ...
      <section className='container py-10 grid grid-cols-1 lg:grid-cols-4 gap-10 mx-auto max-w-6xl'>
        ...
        {/* Sidebar */}
        <Sidebar categories={sidebarData.categories} recentPosts={sidebarData.recentPosts} tags={sidebarData.tags} />
      </section>
    </div>
  );
}

...
const Block = ({ block }: { block: BlockNode }) => {
  ...
  switch (block.type) {
    case "paragraph":
      return <p className='mb-4 leading-relaxed text-gray-700'>{renderChildren(block.children)}</p>;
    ...
    case "heading":
      ...
      return <Tag className={styles[block.level || 2]}>{renderChildren(block.children)}</Tag>;

    case "list":
      const ListTag = block.format === "ordered" ? "ol" : "ul";
      const listStyles =
        block.format === "ordered" ? "list-decimal pl-6 mb-4 space-y-2" : "list-disc pl-6 mb-4 space-y-2";
      return <ListTag className={listStyles}>{renderChildren(block.children)}</ListTag>;
    ...
    case "image":
      ...
      return (
        <div className='relative w-full h-auto my-8 overflow-hidden rounded-xl'>
          ...
          />
          {block.image.caption && (
            <p className='mt-2 text-sm text-center text-gray-500 italic'>{block.image.caption}</p>
          )}
        </div>
      );

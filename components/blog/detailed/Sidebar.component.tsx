...
export default function Sidebar({ categories = [], recentPosts = [], tags = [] }: SidebarProps) {
  ...
  return (
    <aside className='space-y-8'>
      {/* Search */}
      <form onSubmit={handleSearch} className='rounded-2xl border p-5'>
        ...
        <ul className='space-y-3'>
          {categories.map((cat) => (
            <li key={cat.id} className='flex justify-between text-sm'>
              ...
              <span className='bg-gray-100 px-2 rounded-full text-xs'>{cat.blogsCount}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className='rounded-2xl border p-5'>
        ...
        <ul className='space-y-4'>
          {recentPosts.map((post) => (
            <li key={post.id} className='flex items-start justify-center gap-3'>
              {/* Image */}
              ...
              <div className='flex-1 min-w-0'>
                <Link href={`/blog/${post.slug}`} className='block text-sm font-medium leading-snug line-clamp-2'>
                  {post.title}
                </Link>
      ...
      <div className='rounded-2xl border p-5'>
        ...
        <div className='flex flex-wrap gap-2'>
          {tags.map((tag) => (
            <Link key={tag.id} href={`/blog/tag/${tag.slug}`} className='bg-gray-100 px-3 py-1 rounded-full text-xs'>
              {tag.name}
            </Link>
          ))}
        </div>

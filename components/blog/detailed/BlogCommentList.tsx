...
function BlogCommentItem({ comment, blogDocumentId, depth = 0 }: BlogCommentItemProps) {
    ...
    <div>
      <div className='flex gap-4 mb-4'>
        ...
        <div className='flex-1'>
          ...
          {depth < maxDepth && (
            ...
            >
              <ArrowBendDownRightIcon weight='bold' size={20} /> Reply
            </button>
          )}
      ...
      {replies.length > 0 && (
        <div className='ml-14 border-l pl-6 space-y-6'>
          {replies.map((reply) => (
            <BlogCommentItem key={reply.id} comment={reply} blogDocumentId={blogDocumentId} depth={depth + 1} />
          ))}
        </div>
      )}
    </div>
...
export function BlogCommentList({ blogDocumentId }: { blogDocumentId: string }) {
  ...
  return (
    <div className='space-y-8 mt-12'>
      <h3 className='text-2xl font-bold'>Comments ({comments?.length || 0})</h3>

      <div className='space-y-8'>
        {comments?.map((comment) => (
          <BlogCommentItem key={comment.id} comment={comment} blogDocumentId={blogDocumentId} />
        ))}

        {(!comments || comments.length === 0) && <p className='italic text-gray-500'>No comments yet.</p>}
      </div>
